`timescale 1ns/1ps
`define N 16
`define TEST_KERNEL_V
`define PATTERN_HORI_STRIPE
...
module top_tb;
    ...
    parameter TEST_SEQUENCE = 0;  // 0: weights first then data, 1: data first then weights
    parameter MAX_WAIT_CYCLES = 300; // maximum wait cycles
    
    // Test data
    logic [7:0] test_input [0:INPUT_SIZE-1];
    logic signed [7:0] test_weight [0:WEIGHT_SIZE-1];
    ...
    // Output capture to txt
      integer fout;
      int col_counter = 0;
      bit output_active = 0;
      logic prev_output_start;
    ...
    // Pattern selection
    initial begin
        `ifdef PATTERN_HORI_EDGE
            for (int i = 0; i < IMG_SIZE; i++) begin
                for (int j = 0; j < IMG_SIZE; j++) begin
                    if (i < IMG_SIZE/4 || i >= 3*IMG_SIZE/4)
                        test_input[i*IMG_SIZE + j] = 8'd0;
                    else
                        test_input[i*IMG_SIZE + j] = 8'd127;
                end
            end
        `elsif PATTERN_VERT_EDGE
            for (int i = 0; i < IMG_SIZE; i++) begin
                for (int j = 0; j < IMG_SIZE; j++) begin
                    if (j < IMG_SIZE/4 || j >= 3*IMG_SIZE/4)
                        test_input[i*IMG_SIZE + j] = 8'd0;
                    else
                        test_input[i*IMG_SIZE + j] = 8'd127;
                end
            end
        `elsif PATTERN_HORI_STRIPE
            for (int i = 0; i < IMG_SIZE; i++) begin
                for (int j = 0; j < IMG_SIZE; j++) begin
                    if ((i / 3) % 2 == 0)
                        test_input[i*IMG_SIZE + j] = 8'd0;
    ...
    initial begin
        ...
        `elsif TEST_KERNEL_V
            ...
            test_weight[8] =  8'sd1;
        `else
            $fatal("Undefined TEST_KERNEL macro");
        `endif
    end
    ...
    always @(posedge clk) begin
    prev_output_start <= DUT.ctrl_inst.output_start;

    // 输出开始
    if (DUT.ctrl_inst.output_start && !prev_output_start) begin
        output_active = 1;
        $display("Output phase started at time %t", $time);
    end

    // 输出进行中：写入数据
    if (output_active && DUT.ctrl_inst.output_start) begin
        $fwrite(fout, "%0d", DUT.ctrl_inst.output_data);
        col_counter++;
        if (col_counter < 14)
            $fwrite(fout, " ");
        else begin
    ...
    // 输出结束：关闭文件
    if (output_active && !DUT.ctrl_inst.output_start && prev_output_start) begin
        $display("Output phase finished at time %t", $time);
        $fclose(fout);
        output_active = 0;
    end
end
